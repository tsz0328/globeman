<template>
  <WorkPage :loading="loading">
    ...
    <RepairInboundDialog v-model="inboundDialogVisible" :order-name="inboundOrderName" :order-id="inboundOrderId"
      :details="inboundDetails" @refresh="handleInboundRefresh" />
  </WorkPage>
</template>
...
import RepairInboundDialog from '@/components/repair/RepairInboundDialog.vue'
...
const handleDetailsChanged = async () => {
  ...
  if (selectedOrder.value) {
    ...
    if (updated) {
      selectedOrder.value = { ...selectedOrder.value!, name: updated.name, customer: updated.customer }
